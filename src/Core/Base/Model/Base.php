...
//                    $logging->log("var_val dump:". var_export($var_val, true), $this->getModuleName()) ;
                    $was_array = false ;
                    if (is_array($var_val) || is_object($var_val)) {
                        $was_array = true ;
                        $var_val = serialize($var_val) ;
                    }
                    $paramValue = str_replace($vid.$var_name, $var_val, $paramValue) ;
                    if ($was_array === true) {
                        $paramValue = unserialize($var_val) ;
                    } } } }
        if (is_array($paramValue)) {
            return $paramValue ;
        }
...
//                var_dump("vdd:", $sc) ;
    ...
    protected function swapResForVariable($res, $paramValue, $parts_string) {
        ...
        $loggingFactory = new \Model\Logging();
        $logging = $loggingFactory->getModel(array());
//        $logging->log("res dump:". var_export($res, true), $this->getModuleName()) ;
//        if (is_array($res)) {
//                var_dump($res) ;
//        die() ;
//            return $res ;
