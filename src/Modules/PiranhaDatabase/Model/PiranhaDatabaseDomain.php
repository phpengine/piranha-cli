...
class PiranhaDatabaseDomain extends BasePiranhaDatabaseAllOS {
    ...
    public function askWhetherToCreateHostedZone($params=null) {
        return $this->performPiranhaDatabaseCreateHostedZone($params);
    }

    public function askWhetherToCreateHealthCheck($params=null) {

        return $this->performPiranhaDatabaseCreateHealthCheck($params);
    }
    public function askWhetherToDeleteHostedZone($params=null) {

        return $this->performPiranhaDatabaseDeleteHostedZone($params);
    }

    public function askWhetherToDeleteHealthCheck($params=null) {

        return $this->performPiranhaDatabaseDeleteHealthCheck($params);
    }
    protected function performPiranhaDatabaseCreateHealthCheck($params=null){
    ...
    protected function performPiranhaDatabaseCreateHostedZone($params=null){
        if ($this->askForAddExecute() != true) { return false; }
        $this->initialisePiranha();
        $this->getDomainName();
...
//
            ...
            if ($domainExists !== false) {
                ...
                $p_api_vars['api_uri'] = '/api/database/domain/create';
...
//        $route53Result->ResourceRecordSetCount = $single_hosted_zone['ResourceRecordSetCount'] ;
    ...
    protected function doesDomainExist() {
        $p_api_vars['api_uri'] = '/api/database/domain/all';
        $p_api_vars['page'] = 'all' ;
...
//        var_dump($list);
        ...
        foreach ($list['domains'] as $domain) {
            if ($domain['database_domain_name'] === $this->params["domain-name"]) {
                $found['domain_name'] = $domain['database_domain_name'] ;
                $found['id'] = $domain['database_domain_id'] ;
            }
        }
        return $found ;
    }

    protected function performPiranhaDatabaseDeleteHealthCheck($params=null){
    ...
    protected function performPiranhaDatabaseDeleteHostedZone($params=null){
        if ($this->askForDeleteExecute() != true) { return false; }
        $this->initialisePiranha();
        $this->getDomainName();
        ...
        try{
            ...
            if (isset($this->params["hosted-zone-id"]) && $this->params["hosted-zone-id"]  !== '') {
                ...
                $result = $this->piranhaClient->listHostedZonesByName(
                    array(
                        'DatabaseName' =>$this->params["domain-name"],
                        'CallerReference'=>$unique
                    )
                ) ;
            } else {
