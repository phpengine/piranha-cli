...
class PiranhaVPCDomain extends BasePiranhaVPCAllOS {
    ...
    public function askWhetherToCreateHostedZone($params=null) {
        return $this->performPiranhaSLBCreateHostedZone($params);
    }

    public function askWhetherToCreateHealthCheck($params=null) {

        return $this->performPiranhaSLBCreateHealthCheck($params);
    }
    public function askWhetherToDeleteHostedZone($params=null) {

        return $this->performPiranhaSLBDeleteHostedZone($params);
    }

    public function askWhetherToDeleteHealthCheck($params=null) {

        return $this->performPiranhaSLBDeleteHealthCheck($params);
    }
    protected function performPiranhaSLBCreateHealthCheck($params=null){
    ...
    protected function performPiranhaSLBCreateHostedZone($params=null){
        if ($this->askForAddExecute() != true) { return false; }
        $this->initialisePiranha();
        $this->getDomainName();
...
//
            ...
            if ($domainExists !== false) {
                ...
                $p_api_vars['api_uri'] = '/api/slb/domain/create';
...
//        $route53Result->ResourceRecordSetCount = $single_hosted_zone['ResourceRecordSetCount'] ;
    ...
    protected function doesDomainExist() {
        $p_api_vars['api_uri'] = '/api/slb/domain/all';
        $p_api_vars['page'] = 'all' ;
...
//        var_dump($list);
        ...
        foreach ($list['domains'] as $domain) {
            if ($domain['slb_domain_name'] === $this->params["domain-name"]) {
                $found['domain_name'] = $domain['slb_domain_name'] ;
                $found['id'] = $domain['slb_domain_id'] ;
            }
        }
        return $found ;
    }

    protected function performPiranhaSLBDeleteHealthCheck($params=null){
    ...
    protected function performPiranhaSLBDeleteHostedZone($params=null){
        if ($this->askForDeleteExecute() != true) { return false; }
        $this->initialisePiranha();
        $this->getDomainName();
        ...
        try{
            ...
            if (isset($this->params["hosted-zone-id"]) && $this->params["hosted-zone-id"]  !== '') {
                ...
                $result = $this->piranhaClient->listHostedZonesByName(
                    array(
                        'SLBName' =>$this->params["domain-name"],
                        'CallerReference'=>$unique
                    )
                ) ;
            } else {
