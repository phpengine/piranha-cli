...
class PiranhaSCMList extends BasePiranhaSCMAllOS {
    ...
    public function askWhetherToListData($params=null) {
        ...
        return $this->performPiranhaSMPListData($params);
    }

    protected function performPiranhaSMPListData($params=null){
        if ($this->askForListExecute() != true) { return false; }
        $this->initialisePiranha();
        $dataToList = $this->askForDataTypeToList();
        return $this->getDataListFromPiranhaSMP($dataToList);
    }
    ...
    public function getDataListFromPiranhaSMP($dataToList) {
        $list = [] ;
        try {
...
//            }
            if(in_array($dataToList, array('records', 'Records'))) {
                $p_api_vars['api_uri'] = '/api/smp/record/all';
                $p_api_vars['page'] = 'all' ;
                $list = $this->performRequest($p_api_vars);
            }
        } catch (\Exception $e) {
