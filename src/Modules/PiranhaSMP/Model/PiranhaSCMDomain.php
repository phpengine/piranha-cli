...
class PiranhaSCMDomain extends BasePiranhaSCMAllOS {
    ...
    public function askWhetherToCreateHostedZone($params=null) {
        return $this->performPiranhaSMPCreateHostedZone($params);
    }

    public function askWhetherToCreateHealthCheck($params=null) {

        return $this->performPiranhaSMPCreateHealthCheck($params);
    }
    public function askWhetherToDeleteHostedZone($params=null) {

        return $this->performPiranhaSMPDeleteHostedZone($params);
    }

    public function askWhetherToDeleteHealthCheck($params=null) {

        return $this->performPiranhaSMPDeleteHealthCheck($params);
    }
    protected function performPiranhaSMPCreateHealthCheck($params=null){
    ...
    protected function performPiranhaSMPCreateHostedZone($params=null){
        if ($this->askForAddExecute() != true) { return false; }
        $this->initialisePiranha();
        $this->getDomainName();
...
//
            ...
            if ($domainExists !== false) {
                ...
                $p_api_vars['api_uri'] = '/api/smp/domain/create';
...
//        $route53Result->ResourceRecordSetCount = $single_hosted_zone['ResourceRecordSetCount'] ;
    ...
    protected function doesDomainExist() {
        $p_api_vars['api_uri'] = '/api/smp/domain/all';
        $p_api_vars['page'] = 'all' ;
...
//        var_dump($list);
        ...
        foreach ($list['domains'] as $domain) {
            if ($domain['smp_domain_name'] === $this->params["domain-name"]) {
                $found['domain_name'] = $domain['smp_domain_name'] ;
                $found['id'] = $domain['smp_domain_id'] ;
            }
        }
        return $found ;
    }

    protected function performPiranhaSMPDeleteHealthCheck($params=null){
    ...
    protected function performPiranhaSMPDeleteHostedZone($params=null){
        if ($this->askForDeleteExecute() != true) { return false; }
        $this->initialisePiranha();
        $this->getDomainName();
        ...
        try{
            ...
            if (isset($this->params["hosted-zone-id"]) && $this->params["hosted-zone-id"]  !== '') {
                ...
                $result = $this->piranhaClient->listHostedZonesByName(
                    array(
                        'SMPName' =>$this->params["domain-name"],
                        'CallerReference'=>$unique
                    )
                ) ;
            } else {
