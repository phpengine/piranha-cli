...
class EnvironmentConfigInfo extends PTConfigureBase {
  ...
  EnvironmentConfig, environmentconfig, environment-config, envconfig, env-config
        ...
        example: piranha envconfig list --yes

        - list-local
        List current local environments
        example: piranha envconfig list-local --yes

        - configure, config
        Configure bespoke environments for your project to use
        example: piranha envconfig config
        # below to create an empty environment to add instances to
        example: piranha envconfig config --yes
        ...
        example: piranha envconfig config-default
        example: piranha envconfig config-default --yes --environment-name="local-80/local-8080"

        - delete, del
        Configure the environments for your project to use
        example: piranha envconfig delete
        example: piranha envconfig del --environment-name="staging"
