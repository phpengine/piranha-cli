...
class PiranhaDNSDomain extends BasePiranhaDNSAllOS {
    ...
    public function askWhetherToCreateHostedZone($params=null) {
        return $this->performPiranhaDNSCreateHostedZone($params);
    }

    public function askWhetherToCreateHealthCheck($params=null) {

        return $this->performPiranhaDNSCreateHealthCheck($params);
    }
    public function askWhetherToDeleteHostedZone($params=null) {

        return $this->performPiranhaDNSDeleteHostedZone($params);
    }

    public function askWhetherToDeleteHealthCheck($params=null) {

        return $this->performPiranhaDNSDeleteHealthCheck($params);
    }
    protected function performPiranhaDNSCreateHealthCheck($params=null){
    ...
    protected function performPiranhaDNSCreateHostedZone($params=null){
        if ($this->askForAddExecute() != true) { return false; }
        $this->initialisePiranha();
        $this->getDomainName();
...
//
            ...
            if ($domainExists !== false) {
                ...
                $p_api_vars['api_uri'] = '/api/dns/domain/create';
...
//        var_dump($list);
        ...
        foreach ($list['domains'] as $domain) {
            if ($domain['dns_domain_name'] === $this->params["domain-name"]) {
                $found['domain_name'] = $domain['dns_domain_name'] ;
                $found['id'] = $domain['dns_domain_id'] ;
            }
        }
        return $found ;
    }

    protected function performPiranhaDNSDeleteHealthCheck($params=null){
    ...
    protected function performPiranhaDNSDeleteHostedZone($params=null){
        if ($this->askForDeleteExecute() != true) { return false; }
        $this->initialisePiranha();
        $this->getDomainName();
        ...
        try{
            ...
            if (isset($this->params["hosted-zone-id"]) && $this->params["hosted-zone-id"]  !== '') {
                ...
                $result = $this->piranhaClient->listHostedZonesByName(
                    array(
                        'DNSName' =>$this->params["domain-name"],
                        'CallerReference'=>$unique
                    )
                ) ;
            } else {
